...
namespace SAOD6
{
    public class BadTree<T> where T : IComparable
    {
        ...
        private BadTreeNode<T> root { get => _root; set { _root = value; } }
        ...
        {
            if (_root == null)
                _root = new BadTreeNode<T>(x);
            else
                Insert(_root, x);
        }

        private BadTreeNode<T> Insert(BadTreeNode<T> p, T x)
        {
            if (p == null)
                return new BadTreeNode<T>(x);
            ...
            p.Height = 1 + Math.Max(Heiht(p.Left), Heiht(p.Right));
            int b = balance(p);
            if (b > 1 && x < p.Left.x)
                return RightRot(p);
            if (b < -1 && x > p.Right.x)
            ...
            if(b < -1 && x > p.Right.x)
                p.Right = RightRot(p.R);
            return LeftRot(p);
        }


        public BadTreeNode<T> RightRot(BadTreeNode<T> y)
        {
            var x = y.Left;
            var t = x.Right;
            x.Right = y;
            y.Left = t;
            y.Height = 1 + Math.Max(Heiht(y.Left), Heiht(y.Right));
            x.Height = 1 + Math.Max(Heiht(x.Left), Heiht(x.Right));
            return x;
        }
        public BadTreeNode<T> LeftRot(BadTreeNode<T> y)
        {
            var y = x.Right;
            var t = y.Left;
            x.Right = y;
